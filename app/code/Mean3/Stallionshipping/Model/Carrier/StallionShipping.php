<?php
/*
 * @package     stallion_tracking
 */

namespace Mean3\Stallionshipping\Model\Carrier\StallionShipping;

use Magento\Quote\Model\Quote\Address\RateRequest;

class StallionShipping
extends \Magento\Shipping\Model\Carrier\AbstractCarrier
implements \Magento\Shipping\Model\Carrier\CarrierInterface
{

protected $_code = 'stallion_tracking';
...
{
    return ['stallion_tracking' => $this->getConfigData ('name')];
}
...
protected function _getTracking($trackings)
{
    ...
    foreach ($trackings as $tracking)
    {
        ...
        $status->setTrackSummary($this->_getStallionCustomTracking($trackUrl));
        $status->setPopup(1);
...
/**
 * Get Mean3 tracking information
 *
 * @param string $url
 * @return string|false
 * @api
 */
public function _getStallionCustomTracking($url)
{
    if(empty($url)) return;
