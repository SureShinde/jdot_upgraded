...
namespace RLTSquare\CustomPrint\Model;
...
/**
 * Class Carrier
 * @package RLTSquare\CustomPrint\Model
 */
class Carrier extends \Magento\Dhl\Model\AbstractDhl implements \Magento\Shipping\Model\Carrier\CarrierInterface
{
    ...
    public function __construct(
        ...
        \RLTSquare\CustomPrint\Helper\Data $helperData
            ...
            foreach ($reverseOrderItems as $key => $weight) {
                ...
                foreach ($items as $key => $weight) {
                    if ($sumWeight + $weight < $maxWeight) {
                        ...
                        $nodePiece->addChild('Weight', sprintf('%.2f', $sumWeight));
                        break;
                    } else {
                        unset($items[$key]);
                        $numberOfPieces++;
    ...
    protected function _doRequest()
    {
         ...
         */
        $reference = $this->helper->getGeneralConfig('reference_id');
        $nodeReference = $xml->addChild('Reference', '', '');
        $nodeReference->addChild('ReferenceID', $reference);
        ...
        $company = $this->helper->getGeneralConfig('company_name');
        $nodeShipper->addChild('CompanyName', $company);
    ...
    protected function _shipmentDetails($xml, $rawRequest, $originRegion = '')
    {
        ...
        foreach ($rawRequest->getPackages() as $package){
            ...
            $nodePiece->addChild('Weight', sprintf('%.2f', $package['params']['weight']));
            ...
            $dhl_content = array();
            /**
             * a flag that make sure only two categories should get printed
             */
            $shouldTwoProdCategoriesFlag = 0;
            foreach ($package['items'] as $item) {
                $shouldTwoProdCategoriesFlag++;
                $categoryId=[];
                $content[] = $item['name'];
                ...
                $dhl_content[] = $_product->getData('dhl_content');

                //$keys= array_slice($categoryId[0], -2);
                $dhlContent= array_slice($dhl_content, -2);
//                foreach ($keys as $key) {
//                    $category = $this->category->load($key);
...
//                }
        ...
        /**
         * make unique categories only
         */
        if(isset($dhlContent)){
            $dhlContent = array_unique($dhlContent);
            /**
             * If more than two then the others will get removed
             */
            if ($shouldTwoProdCategoriesFlag > 2 && count($dhlContent) == 2) {
                $dhlContent = array_slice($dhlContent, 0, 4);
            }
            $productString = implode(',', $dhlContent);
            /**
             * If still the string more than 90 then cut off the string into words and attach only two
             */
            if (strlen($productString) > 89 && count($dhlContent) == 2) {
                $parts = explode(',', $productString);
                $partsa[] = array($parts[0], $parts[1], $parts[2], $parts[3]);
                $productString = implode(',', $partsa[0]);
                $updatedLength = strlen($productString);
            } /**
             * If still greater than 90 then trim the string but don't cut off words
             */
            if (strlen($productString) > 89) {
                //$productString = substr($productString, 0, strpos($productString, ' ', 89));
                $productString = substr($productString, 0,89);
            }
        }else{
            $productString = "";
        }
        $nodeShipmentDetails->addChild('Weight', sprintf('%.2f', $rawRequest->getPackageWeight()));
        ...
        $nodeShipmentDetails->addChild('Contents', $productString);
        /**
