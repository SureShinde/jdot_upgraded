<?php
/**
 * Overriding the Create order save controller to save order cancel reason
 *
 * @author  Dawood Gondal <dawood.gondal@rltsquare>
 */

namespace RLTSquare\CancelOrder\Controller\Adminhtml\Order\Create;
...
/**
 * Class SaveOverride
 * @package RLTSquare\CancelOrder\Controller\Adminhtml\Order\Create
 */
class SaveOverride extends \Magento\Sales\Controller\Adminhtml\Order\Create implements HttpPostActionInterface
{
    /**
    ...
    public function execute()
    {
        ...
        try {
            ...
            // Saving order cancel reason of old order
            $oldOrder = $this->_getSession()->getOrder();
            if ($oldOrder->getPayment()->getMethod() == "etisalatpay") {
                $oldOrder->setOrderCancelReason(
                    $this->getRequest()->getParam('order')['reason']
                );
                $oldOrder->setOrderCancelDescription(
                    $this->getRequest()->getParam('order')['reason_detail']
                );
