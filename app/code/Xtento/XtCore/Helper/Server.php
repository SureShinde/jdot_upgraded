...
class Server extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $url = 'uk.junaidjamshed.com/';// str_replace(['http://', 'https://', 'www.'], '', $this->scopeConfig->getValue('web/unsecure/base_url'));
        $url = explode('/', $url);
        $url = array_shift($url);
    ...
    {
        $url = 'uk.junaidjamshed.com/';//str_replace(['http://', 'https://', 'www.'], '', $this->_request->getServer('SERVER_NAME'));
        $url = explode('/', $url);
        $url = array_shift($url);
        $parsedUrl = parse_url($url, PHP_URL_HOST);
        if ($parsedUrl !== null) {
