...
use Magento\Logging\Model\ArchiveFactory;
            ...
            if ($product->getId()) {
                foreach ($product->getAttributes(null, true) as $productAttribute) {
                    ...
                    if (!empty($value) && $productAttribute->usesSource() && ($options = $productAttribute->getSource()->getAllOptions()) && !empty($options)) {
                        if (sizeof($options) > 0) {
                            if($attributeCode == "bin_location" || $attributeCode == "bin_location_store"){
                                $tmpValues = explode(',', $value);
                                $kvp = Array();
                                $tmpLabels = Array();
                                foreach ($options as $option) {
                                    if (isset($option['value'])) {
                                        $kvp[$option['value']] = (string)$option['label'];
                                    }
                                }

                                foreach ($tmpValues as $tmpValue) {
                                    $tmpLabels[] = $kvp[$tmpValue];
                                }

                                $value = implode(', ',$tmpLabels);
                            }
                            else{
                                foreach ($options as $option) {
