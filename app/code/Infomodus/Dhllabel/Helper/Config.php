...
class Config extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function createMediaFolders()
    {
        ...
        $path_upsdir = $baseMediaDir . '/dhllabel';
        if (!is_dir($path_upsdir)) {
            mkdir($path_upsdir, 0777);
        }
        $path_upsdir = $baseMediaDir . '/dhllabel/label';
        if (!is_dir($path_upsdir)) {
            mkdir($path_upsdir, 0777);
        }
        $path_upsdir = $baseMediaDir . '/dhllabel/test_xml';
        if (!is_dir($path_upsdir)) {
            mkdir($path_upsdir, 0777);
        }
    ...
    public function escapeXML($string)
    {
        ...
        //$string = preg_replace('/"/is', '&quot;', $string);
        $string = preg_replace('/’/is', '&#8217;', $string);
        if ($this->getStoreConfig('dhllabel/additional_settings/umlaut') == 1) {
            $string = str_replace(
    ...
    {
        try {
            $ip = trim($this->getStoreConfig('dhllabel/printing/automatic_printing_ip', $storeId));
            $port = trim($this->getStoreConfig('dhllabel/printing/automatic_printing_port', $storeId));
    ...
    public function getShippingMethods($storeId = null)
    {
        ...
        foreach ($_methods as $_carrierCode => $_carrier) {
            if ($_carrierCode !== "ups" && $_carrierCode !== "dhl"
                && $_carrierCode !== "usps"
