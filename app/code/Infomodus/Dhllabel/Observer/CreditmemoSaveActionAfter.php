...
class CreditmemoSaveActionAfter
{
    ...
    )
    {
        ...
        if ($this->_coreRegistry->registry('dhllabel_order_id') !== null) {
            ...
            if ($paramShipment !== null
                && isset($paramShipment['infomodus_dhl_label'])
                && $paramShipment['infomodus_dhl_label'] == 1
            ) {
                return $this->redirect->redirect($subject->getResponse(), $this->url->getUrl('infomodus_dhllabel/items/edit', ['order_id' => $order_id, 'shipment_id' => $shipment_id, 'direction' => 'refund', 'redirect_path' => 'creditmemo']));
            }
        }
