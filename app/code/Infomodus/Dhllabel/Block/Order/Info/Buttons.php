 ...
 */
namespace Infomodus\Dhllabel\Block\Order\Info;
...
class Buttons extends \Magento\Framework\View\Element\Template
{
    ...
    protected $_conf;
    /**
     * @var \Infomodus\Dhllabel\Model\ResourceModel\Items\Collection
     */
    private $labels;
    ...
    public function __construct(
        ...
        \Infomodus\Dhllabel\Model\ResourceModel\Items\Collection $labels,
        array $data = []
    ) {
    ...
    public function getRMAUrl($order)
    {
        if ($order->getState() !== \Magento\Sales\Model\Order::STATE_COMPLETE
            || $this->_conf->getStoreConfig('dhllabel/return/refundaccess')==0
            || $this->_conf->getStoreConfig('dhllabel/return/frontend_customer_return')==0
        ) {
            return null;
        }
        return $this->getUrl('dhllabel/rma/edit', ['order_id' => $order->getId()]);
    }
    ...
    {
        if ($this->_conf->getStoreConfig('dhllabel/return/frontend_customer_return') == 0) {
            return [];
        }
