...
class Show extends \Infomodus\Dhllabel\Controller\Adminhtml\Items
{
    public function execute()
    {
        ...
        $model = [];
        if ($label_ids !== '') {
            $labels = $this->itemsFactory->create()->getCollection()->addOrder('created_time', 'DESC')->addFieldToFilter('dhllabel_id', ['in' => explode(',', $label_ids)]);
            $redirectLink = $this->getUrl('infomodus_dhllabel/*/');
            $model['back_link'] = $redirectLink;
        } elseif ($order_id !== null || $shipment_id !== null) {
            $labels = $this->itemsFactory->create()->getCollection()->addOrder('created_time', 'DESC');
            if ($order_id !== null) {
        ...
        if (count($labels) > 0) {
            ...
            $this->_coreRegistry->register('infomodus_dhllabel_items_show', $model);
            $this->_initAction();
            $this->_view->getLayout()->getBlock('items_items_show');
            $this->_view->renderLayout();
        } else {
             $this->_redirect($this->getUrl('infomodus_dhllabel/*'));
        }
