require(["jquery"], function ($) {
    $(document).ready(function () {
        if ($('#ship_items_container .admin__page-section-item.order-totals.order-totals-actions .admin__page-section-item-content').size() > 0) {
            ...
            <input id="infomodus_dhl_label" class="admin__control-checkbox" name="shipment[infomodus_dhl_label]" value="1" type="checkbox">\
            <label class="admin__field-label" for="infomodus_dhl_label">\
            <span>Create DHL label</span></label>\
        </div>');
        }
        ...
        if ($('#creditmemo_item_container .admin__page-section-item.order-totals.creditmemo-totals .order-totals-actions').size() > 0) {
            ...
            <input id="infomodus_dhl_label" class="admin__control-checkbox" name="creditmemo[infomodus_dhl_label]" value="1" type="checkbox">\
            <label class="admin__field-label" for="infomodus_dhl_label">\
            <span>Create DHL label</span></label>\
        </div>');
        }
