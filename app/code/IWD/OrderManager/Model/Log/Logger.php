...
class Logger
{
    ...
    public function __construct(
        ...
        Session $authSession,
        PsrLogger $psrLogger,
        \RLTSquare\EmailContent\Helper\Data $helperData
    ...
    public function saveLogsAsOrderComments($order)
    {
        ...
        if($order->getStatus() == "s_order_confirmed"){
            $comment = $this->helper->getGeneralConfig('email_content');
//            $comment = "Dear Valued Customer, Your order has been confirmed. The delivery time for sale orders is 5-6 working days. In case of any queries, you can reply to this email.";
	}
        if (empty($comment)) {
            return;
        }
        ...
        try {
	      ...
	      $notify = $visible = true;
            ...
            $history->save();
	   if($order->getStatus() == "s_order_confirmed"){
	      $orderCommentSender = ObjectManager::getInstance()->create('Magento\Sales\Model\Order\Email\Sender\OrderCommentSender');
              $orderCommentSender->send($order, $notify, "Dear Valued Customer, Your order has been confirmed. The delivery time for sale orders is 5-6 working days. In case of any queries, you can reply to this email.");
	   }
        } catch (\Exception $e) {
