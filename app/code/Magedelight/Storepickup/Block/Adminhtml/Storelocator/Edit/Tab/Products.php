...
use Magedelight\Storepickup\Model\StorelocatorFactory as StorelocatorFactory;
...
class Products extends ExtendedGrid implements TabInterface
{
    ...
    {
        $selected = $this->getStoreConditionProduct();
    ...
    public function getStoreConditionProduct()
    {
        $storeId = $this->getRequest()->getParam('id');
        //$actAttribute = $this->getRequest()->getParam('act');
        $storeModel   = $this->storelocatorFactory->create();
        if ($storeId) {
            $storeModel->load($storeId);
        }
        
        $conditionsProductsIds = explode(',', $storeModel->getProductIds());
