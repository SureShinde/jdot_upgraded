...
class Storelocator extends Template
{
    ...
    /**
     * @ Allow products to store
     */
    const XML_PATH_ALLOW_PRODUCT_STORE = 'magedelight_storepickup/storeinfo/allowproductallstore';
     ...
     * @return type string
     */
    public function getProductAllowTostore()
    {
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $_allowProduct = $this->scopeConfig->getValue(self::XML_PATH_ALLOW_PRODUCT_STORE, $storeScope);
        return $_allowProduct;
    }

    /*
     * To get current product id from quote
     */
    ...
    /*
     * To get avaliable store list for product and display on checkout page.
     */
    public function getAvailabelStoreListForCurrentProduct()
    {
        $ids = $this->getProductFromQuote();
        if ($ids) {
            $md_store_id = $this->_storeManager->getStore()->getId();
            $storelocatorModel = $this->_modelstorelocatorFactory->create();
            $storelocatorCollection = $storelocatorModel->getCollection()
    ...
    {
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $_Off_Day = $this->scopeConfig->getValue('carriers/storepickup/offdays', $storeScope);
        if($_Off_Day < 1){
            $_Off_Day = 0;
        }else{
            $_Off_Day = $_Off_Day;
