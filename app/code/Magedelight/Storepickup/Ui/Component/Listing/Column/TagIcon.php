...
class TagIcon extends \Magento\Ui\Component\Listing\Columns\Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                if (!empty($item['tag_icon'])) {
                    $item[$fieldName . '_src'] = $this->imageModel->getBaseUrl() . $item[$fieldName];
                    $item[$fieldName . '_alt'] = $item['tag_name'];
                    $item[$fieldName . '_title'] = $item['tag_name'];
                    $item[$fieldName . '_link'] = $this->urlBuilder->getUrl(
                        'storepickupadmin/tag/edit',
                        ['tag_id' => $item['tag_id'], 'store' => $this->context->getRequestParam('store')]
                    );
