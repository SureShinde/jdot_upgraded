            ...
            $.each(config.options, function () {
                ...
                if (!optionConfig.hasOwnProperty(this.id)) {
                    ...
                    if(config.code == 'color')
                        type = 1;
                    else if(config.code == 'size' || config.code =='shoe_size')
                        type = 0;

                    value = this.label;//config[id].hasOwnProperty('value') ? config[id].value : '';
                    var matches = value.match(/\b(\w)/g);              // ['J','S','O','N']
                    var acronym = matches.join('');                  // JSON
                    value = acronym;
                    thumb = '';//config[id].hasOwnProperty('thumb') ? config[id].thumb : '';
